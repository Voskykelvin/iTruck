...
async function request(path, options = {}, retry = true) {
  ...
  const headers = isForm
    ? { 'X-Device-Id': getDeviceId(), ...(token() ? { Authorization: `Bearer ${token()}` } : {}), ...(options.headers || {}) }
    : {
...
export const api = {
  ...
  login: payload => request('/auth/login', { method: 'POST', body: JSON.stringify({ ...payload, deviceId: getDeviceId() }) }),
  ...
  register: (role, payload) => request(`/auth/register/${role}`, { method: 'POST', body: JSON.stringify({ ...payload, deviceId: getDeviceId() }) })
};
