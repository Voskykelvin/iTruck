...
function normalizeTruck(truck) {
  ...
  const routeFit = Number(truck.routeFit || Math.min(98, 62 + (verified ? 18 : 0) + Math.min(12, routeCount * 4) + Math.min(8, Math.floor(trips / 35))));
...
function getFilteredTrucks() {
  ...
  const filtered = trucks.filter(truck => {
    ...
    return (!type || truck.type === type) && (!verifiedOnly || truck.verified) && truck.ratingNumber >= minRating && haystack.includes(q);
  });
...
function renderLoading() {
  ...
  document.getElementById('truckResults').innerHTML = [1, 2, 3].map(() => `
...
function render() {
  ...
  document.getElementById('resultCount').textContent = `${filtered.length} ${filtered.length === 1 ? 'truck' : 'trucks'} found`;
  ...
  document.getElementById('truckResults').innerHTML = filtered.map(truck => `
    <article class="truck-card">
      ...
      <div class="profile-routes">${(truck.routes || []).slice(0, 2).map(route => `<span>${route}</span>`).join('') || '<span>Route on request</span>'}</div>
...
document.getElementById('typeChips').addEventListener('click', event => {
  ...
  document.querySelectorAll('#typeChips button').forEach(button => button.classList.toggle('active', button === chip));
  render();
});
...
document.getElementById('resetFilters').addEventListener('click', () => {
  ...
  document.querySelectorAll('#typeChips button').forEach(button => button.classList.toggle('active', button.dataset.type === ''));
  render();
});
