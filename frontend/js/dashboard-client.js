...
const bidProfiles = {
  james: {
    ...
    amount: '$2,860',
    equipment: 'Verified lorry',
    pickup: 'Today 16:00',
    transit: '2d 8h',
    score: 'Best fit',
    insurance: 'Active cargo cover',
    notes: 'GPS, cross-border documents, strong East Africa lane history.'
  },
  grace: {
    truckPlate: 'KCB 991T',
    owner: 'Grace Wanjiku',
    ...
    transit: '2d 4h',
    score: 'Fastest transit',
    insurance: 'Container and long-haul cover',
    notes: 'Higher rate, stronger trailer capacity and long-distance compliance.'
  },
  tunde: {
    truckPlate: 'LAG 882B',
    ...
    amount: '$2,770',
    equipment: 'Mixed cargo bus',
    pickup: 'Tomorrow 12:00',
    transit: '3d 2h',
    score: 'Lowest price',
    insurance: 'Standard goods cover',
    notes: 'Lower cost, pending verification, better for mixed cargo and flexible timing.'
  }
};
...
function openBidReview(title, bids) {
  ...
  document.querySelectorAll('[data-select-bid]').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      document
        .querySelectorAll('[data-bid-card]')
        .forEach((card) => card.classList.toggle('selected', card.dataset.bidCard === key));
...
document.getElementById('shipmentList').innerHTML = shipments
  .map(
    (item, index) => `
...
document.getElementById('quoteList').innerHTML = quotes
  .map(
    (item, index) => `
...
document.getElementById('actionQueue').innerHTML = actions
  .map(
    (item, index) => `
...
document.addEventListener('click', (event) => {
  ...
  const shipmentIndex =
    event.target.dataset.shipmentAction ??
  ...
  if (quoteIndex !== undefined) {
    ...
    openBidReview(`Review bids for ${quote.lane}`, {
  ...
  if (taskIndex !== undefined) {
    ...
    if (task.type === 'bids') {
      openBidReview(`${task.bookingId} bid comparison`, {
